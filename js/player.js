...
function get_chi_mianzi(shoupai, p) {
    ...
    const color = p[0], n = p[1] -0 || 5, d = p[2];
    if (d != '-') return mianzi; // ←外せば、どこからでもチーできるようになる。しかし、エラーがでるので、対策が必要
    
    const bingpaiColor = shoupai._bingpai[color];
    var p0 = p[1], p1, p2;
    
/*    if (3 <= n && bingpaiColor[n-2] > 0 && bingpaiColor[n-1] > 0) { // 上チー
        p1 = (n-2 == 5 && bingpaiColor[0] > 0) ? 0 : n-2;
        p2 = (n-1 == 5 && bingpaiColor[0] > 0) ? 0 : n-1;
        if (shoupai._fulou.length == 3
            && bingpaiColor[n] == 1 && 3 < n && bingpaiColor[n-3] == 1) //兵牌が4567mとか4mを食うと確定食替になるので、それを防ぐ
            ;
        else mianzi.push(color + p1 + p2 + (p0+d));
    }
    if (n <= 7 && bingpaiColor[n+1] > 0 && bingpaiColor[n+2] > 0) { // 下チー
        p1 = (n+1 == 5 && bingpaiColor[0] > 0) ? 0 : n+1;
        p2 = (n+2 == 5 && bingpaiColor[0] > 0) ? 0 : n+2;
        if (shoupai._fulou.length == 3
            && bingpaiColor[n] == 1 && n < 7 && bingpaiColor[n+3] == 1)
            ;
        else mianzi.push(color + (p0+d) + p1 + p2);
    }
    if (2 <= n &&  n <= 8 && bingpaiColor[n-1] > 0 && bingpaiColor[n+1] > 0) { // 嵌張チー
        p1 = (n-1 == 5 && bingpaiColor[0] > 0) ? 0 : n-1;
        p2 = (n+1 == 5 && bingpaiColor[0] > 0) ? 0 : n+1;
        mianzi.push(color + p1 + (p0+d) + p2);
    }*/
    if(color=='z'){ // 字牌のチー
        if(n<=4){ // 風牌のチー
            const N_m2=(((n-2)+3)%4)+1, N_m1=(((n-1)+3)%4)+1, N=n, N_p1=(((n+1)+3)%4)+1, N_p2=(((n+2)+3)%4)+1;
            if(bingpaiColor[N_m2]>0&&bingpaiColor[N_m1]>0){
                mianzi.push(color+N_m2+N_m1+(p0+d));
            }
            if(bingpaiColor[N_p1]>0&&bingpaiColor[N_p2]>0){
                mianzi.push(color+(p0+d)+N_p1+N_p2);
            }
            if(bingpaiColor[N_m1]>0&&bingpaiColor[N_p1]>0){
                mianzi.push(color+N_m1+(p0+d)+N_p1);
            }
        }else{ // 三元牌のチー
            const N_m2=(n-4)%3+5, N_m1=(n-3)%3+5, N=n, N_p1=(n-1)%3+5, N_p2=(n)%3+5;
            if(bingpaiColor[N_m2]>0&&bingpaiColor[N_m1]>0){
                mianzi.push(color+N_m2+N_m1+(p0+d));
            }
            if(bingpaiColor[N_p1]>0&&bingpaiColor[N_p2]>0){
        ...
        const N_m2=(((n-2)+8)%9)+1, N_m1=(((n-1)+8)%9)+1, N=n, N_p1=(((n+1)+8)%9)+1, N_p2=(((n+2)+8)%9)+1;
        if(bingpaiColor[N_m2]>0&&bingpaiColor[N_m1]>0){
            p1 = (N_m2 == 5 && bingpaiColor[0] > 0) ? 0 : N_m2; //赤処理
            p2 = (N_m1 == 5 && bingpaiColor[0] > 0) ? 0 : N_m1;//赤処理
            mianzi.push(color+p1+p2+(p0+d));
        }
        if(bingpaiColor[N_p1]>0&&bingpaiColor[N_p2]>0){
            p1 = (N_p1 == 5 && bingpaiColor[0] > 0) ? 0 : N_p1; //赤処理
            p2 = (N_p2 == 5 && bingpaiColor[0] > 0) ? 0 : N_p2;//赤処理
            mianzi.push(color+(p0+d)+p1+p2);
        }
        if(bingpaiColor[N_m1]>0&&bingpaiColor[N_p1]>0){
            p1 = (N_m1 == 5 && bingpaiColor[0] > 0) ? 0 : N_m1; //赤処理
            p2 = (N_p1 == 5 && bingpaiColor[0] > 0) ? 0 : N_p1;//赤処理
            mianzi.push(color+p1+(p0+d)+p2);
        }
    }
...
Majiang.Player.prototype.select_fulou = function(data) {
    ...
    function checkXiangting(shoupai, m, n_xiangting) {
    ...
    const n_xiangting = this.xiangting(this._shoupai);
    if (n_xiangting == 0) return;
 
    for (const m of this.get_gang_mianzi(data)) {
        return checkXiangting(this._shoupai, m, n_xiangting);
    }
    for (const m of this.get_peng_mianzi(data)) {
        return checkXiangting(this._shoupai, m, n_xiangting);
    }
    for (const m of this.get_chi_mianzi(data)) {
        return checkXiangting(this._shoupai, m, n_xiangting);
    }
}
...
Majiang.Player.prototype.select_gang = function() {
    ...
    for (const m of this.get_gang_mianzi()) {
        return checkXiangting(this._shoupai, m, n_xiangting);
    }
}
